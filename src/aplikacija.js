...
function GameOverDispla({bodovi, startButton, show}) {
    ...
    React.useEffect(() => {
		    ...
		    rr.current.addEventListener("mouseenter", funn1);
		    rr.current.addEventListener("mouseleave", funn2);
			...
			return (() => {
		        ...
		        rr.current.removeEventListener("mouseenter", funn1);
		        rr.current.removeEventListener("mouseleave", funn2);
		    });
	}, []);
		
    function funn1(el) {
		dodajStilove(el.target, {borderColor:"green", backgroundColor:"#555555"});
	};
	
	function funn2(el) {
		dodajStilove(el.target, {borderColor:"black", backgroundColor:"#222222"});
	};
	...
	return (
	    <div ref={r1} id="gameover">
	        ...
	        <div id="gameover-el">
	            <div ref={r} className="gameover-gumb" onClick={startButton}>
	                <div className="gameover-gumb1">
	                    <BsArrowCounterclockwise size="30px"/>
	                </div>
	            </div>
	            <div ref={rr} className="gameover-gumb" onClick={()=>{con.opcijeSw(2)}}>
	                <div className="gameover-gumb1">
...
function Gum({klik}) {
	return (
	    <div onClick={klik} id="gumb">
	        <p className="gumb-p">Start Game</p>
	    </div>
	)
}
...
function GumbOpcij({klik}) {
	return (
	    <div onClick={klik} className="gumbopcije">
	        <IoSettingsOutline size="50px"/>
	    </div>
	)
}

var GumbOpcije = React.memo(GumbOpcij);

function GumbLeve({klik}) {
	const [level, setLevel] = React.useState(1);
	const [inic, setInic] = React.useState(false);
	...
	React.useEffect(() => {
		if (inic) {
		    console.log("lokalni level je " + level);
		    klik(level);
		}
	}, [level]);
	
	function promijeniLevel(ev) {
		ev.preventDefault();
		console.log("upravo si kliknuo " + Math.random());
		if (ev.type == "click") {
	...
	return (
	    <div onClick={promijeniLevel} onContextMenu={promijeniLevel} className="gumbopcije">
	        <p className="gumb-p1">Start Level {level}</p>
	    </div>
	)
}

var GumbLevel = React.memo(GumbLeve);
...
// property sljedeci na prazno polje, []
	...
	React.useEffect(() => {
		...
		if (br == 0) {
			...
			if (br < br1)  br1 = br;
			for (var i = 0; i < br1; i++) {
				pp.push(<NextObjekt key={i} objekt={sl[i]}/>);
				//alert(sl[i]);
			}
		    //pp = sl.map((el, i) => {return <NextObjekt key={i} objekt={el}/>});
		}
		setPolje(pp);
...
export class Aplikacija extends React.Component {
	...
	constructor(props) {
		...
		this.state = {
			...
			projekcijaSw: null,  // za true crta projekciju, za false ne crta
			...
			gameRunsSw: false,       // za false igra nije pokrenuta/game over, za true igra je u tijeku
			snapshotDisplay: null,    // sluzi za memoriranje displaya kada ukljucimo pauzu
		    snapshotNext: null        // sluzi za memoriranje next displaya kada ukljucimo pauzu
		}
		...
		this.autoRepeat = null; // autorepeat u msec za lijevi/desni pomak u msec
		this.autoDelay = null;  // autodelay autorepeata za lijevi/desni pomak u msec
		...
		this.brziPadSw = true;  // prekidac koji onemogucuje ponavljanje brzih padova
		
		this.pauzaSw = true;            // za true pauziramo igru, za false je ponovo pokrecemo
		this.soundVolume = null;
		
		this.ref1 = null;
		this.ref2 = null;
		this.engineRef = null;
		this.snapshotDisplay = null;    // sluzi za memoriranje displaya kada ukljucimo pauzu
		this.snapshotNext = null;       // sluzi za memoriranje next displaya kada ukljucimo pauzu
		...
		this.startajIgru = this.startajIgru.bind(this);
		this.togglePauza = this.togglePauza.bind(this);
	...
	componentDidMount() {
		...
		this.soundVolume = this.context.pod["SOUND VOLUME"];
		this.zvuk = new Zvuk(this.soundVolume/100);
	...
	componentDidUpdate(prevProps, prevState) {
		...
		for (let i = 0; i < 240; i++) {
	  	...
	  	this.autoRepeat = this.context.pod["AUTO-REPEAT SPEED"];
	  	this.autoDelay = this.context.pod["AUTO-REPEAT DELAY"];
	  	
	  	if (this.state.projekcijaSw !== this.context.pod["Ghost"]) {
	  	    this.state.projekcijaSw = this.context.pod["Ghost"];
		    var noviObjekt = null;
			if (this.state.projekcija.tip !== null) {
				noviObjekt = {...this.state.projekcija};
		        this.nacrtajObjekt({...noviObjekt, sw: false, sw1: true});
			}
			if (this.state.projekcijaSw && !this.pauzaSw)  this.projecirajObjekt();
		}
		
		if (this.context.sww1 && true) {
			this.context.opcijeSw1(false);
			this.togglePauza();
		}
		
		if (this.context.pod["SOUND VOLUME"] !== this.soundVolume) {
			this.soundVolume = this.context.pod["SOUND VOLUME"];
			console.log("ucitavam nove razine zvuke.");
			this.zvuk = new Zvuk(this.soundVolume/100);
		}
	...
	render() {
		return (
		    <div id="okvir">
		        <div id="el1">
		            <NextDisplay sljedeci={this.state.nextObjects} br={this.context.pod["NEXT PIECES SHOWED"]}/>
		            
		            <GumbLevel klik={ (noviLevel) => {
						this.setState({startniLevel: noviLevel}); 
						this.postaviVrijeme(this.state.level, this.state.gameRunsSw); 
						console.log("globalni level je " + this.state.startniLevel)}}/>
		        ...
		        <div id="el">
		            ...
		            <BrDisplay naslov="Level:" broj={this.state.level+this.state.startniLevel-1}/>
		            <BrDisplay naslov="Lines:" broj={this.state.brLinija}/>
		            <Gumb klik={this.startajIgru}/>
		            <GumbOpcije klik={() => {if (!this.pauzaSw) this.togglePauza(); this.context.opcijeSw(2)}}/>
		        </div>
		        <GameOverDisplay bodovi={this.state.score} startButton={this.startajIgru} show={this.state.showGameOver}/> 
		    </div>
		)
	}
	
	startajIgru() {
		console.log("pokusavam startati igru..." + this.pauzaSw + "   /   " + Math.random());
		if (this.pauzaSw)  this.engine();
	}
	...
	inicirajPocetakIgre() {
		setTimeout(() => {
		    ...
		    this.setState({level: 1, brLinija: 0, score: 0, showGameOver: false, stanja: inicirajPolje(20, 12, 0), gameRunsSw: true});
		    this.inicirajObjekt(this.state.nextObjects[0]);  // I-shape, kvadrat, munja, obrnuta_munja, L-shape, obrL-shape, T-shape
		    this.sljedeciRandomObjekt();
		    //this.vrijeme = 500;
		    this.postaviVrijeme(1, true);
		    this.pauzaSw = false;
	    }, 10);
	}
	...
	gameOver() {
		...
		this.pauzaSw = true;
		this.setState({showGameOver: true, gameRunsSw: false});
	}
	
	togglePauza() {  // ova funkcija za pokrenutu igru je pauzira, a pauziranu je ponovo pokrece
		if (this.pauzaSw  && this.state.gameRunsSw) {
			console.log("gasimo pauzu..." + Math.random());
			this.pauzaSw = false;
			
			
			//if (this.state.gameRunsSw) {
			//if (this.state.snapshotNext === null) {
				
			//} else {
			this.setState({stanja: JSON.parse(JSON.stringify(this.state.snapshotDisplay)), nextObjects: [...this.state.snapshotNext]});
			//}
			//alert("pogledaj display");
			
			setTimeout(() => {
			    var noviObjekt = {...this.state.projekcija};
		        this.nacrtajObjekt({...noviObjekt, sw: false, sw1: true});
		    }, 1);
			...
			this.pauzaSw = true;
			//alert("palimo pauzu..." + Math.random());
			clearTimeout(this.engineRef);
						
			//this.snapshotDisplay = JSON.parse(JSON.stringify(this.state.stanja));
			//this.snapshotNext = [...this.state.nextObjects];
			this.setState({stanja: inicirajPolje(20, 12, 0), nextObjects: [], snapshotDisplay: JSON.parse(JSON.stringify(this.state.stanja)), snapshotNext:[...this.state.nextObjects]});
		}
	}
	...
	postaviVrijeme(lev, sw) {
		this.vrijeme = 500 * 0.8**(lev - 1  +  this.state.startniLevel - 1);
		
		if (sw) {
			console.log("cistimo interval i postavljamo novi sa vremenom " + this.vrijeme + " / " + sw);
		    clearInterval(this.engineRef);
		    this.engineRef = setInterval(() => {
			    if (this.provjeriDoljeObjekt()) {
				    this.spustiObjekt();
			    } else {
		...
		console.log("NOVO VRIJEME JE " + this.vrijeme + " / " + lev + " / " + this.state.startniLevel);
	}
	...
	urusiPuneLinije(linije) {
		...
		//this.vrijeme = 500 * 0.85**(lev - 1  +  this.state.startniLevel - 1);
		if (lev !== this.state.level)  this.postaviVrijeme(lev, true);
	...
	pritisakGumba(ev) {	
	  ...
	  if (!this.pauzaSw) {	
		...
		switch (ev.code) {
			//case "ArrowDown":
			case this.context.pod["SOFT DROP"]:
			    //this.spustiObjekt();
			    ...
			    break;
			//case "ArrowLeft":
			case this.context.pod["MOVE LEFT"]:
			    ...
			    break;
			//case "ArrowRight":
			case this.context.pod["MOVE RIGHT"]:
			    if (this.horizontalniPomak == "n") {
			    ...
			    break;
			//case "ArrowUp":
			case this.context.pod["ROTATE RIGHT"]:
			    if (this.rotirajSw) {
			    ...
			    break;
			//case "KeyZ":
			case this.context.pod["ROTATE LEFT"]:
			    if (this.rotirajSw) {
					this.rotirajSw = false;
			...
			case "KeyD":
		        ...
		        break;
		    //case "Space":
		    case this.context.pod["HARD DROP"]:
		        if (this.brziPadSw) {
					this.brziPadSw = false;
	...
	pritisakGumbaUp(ev) {
		...
		if (ev.code == this.context.pod["PAUSE"])  this.togglePauza();
	  ...
	  if (!this.pauzaSw) {	
		switch (ev.code) {
			...
			//case "Space":
			case this.context.pod["HARD DROP"]:
			    if (!this.brziPadSw)  this.brziPadSw = true;
		}
	  }  // kraj if petlje	
	}
}
